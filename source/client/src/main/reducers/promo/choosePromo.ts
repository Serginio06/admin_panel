...
import {IChoosePromoAction} from "../../actions/promo/choosePromo";
import {PromoRewardType} from "../../../../../types/constants/PromoRewardType";
import {PromoCodeType} from "../../../../../types/constants/PromoCodeType";

export const choosePromoReducer = (state: IState, action: IChoosePromoAction): IState => {
    const promos: ICompanyPromo[] = state.promos.slice();
    const isSamePromo: boolean = action._id === state.promoPageState._id;
    ...
    for (const promo of promos) {
        if (promo._id.toString() === action._id.toString()) {
            targetPromo = promo;
    ...
    if (targetPromo === null) {
        console.error(`Promo wasn't found! PromoId: ${action._id}`);
        return {...state};
    }
    ...
    return {
        ...
        promoPageState: {
            ...
            isEditable: (state.promoPageState.isViewable && isSamePromo) || !isSamePromo,
            isViewable: false,
            _id: action._id,
...
function getStepStatus(promoPageState: ICompanyPromo, steps: IStepsState[]) {
    ...
    steps[2].isCompleted = !!promoPageState.offerPhrase
        ...
        && (!!promoPageState.quantity || promoPageState.isUnlimited)
        && !!promoPageState.time2Decide
        // && (!!promoPageState.codeType
        && (promoPageState.codeType === PromoCodeType.NO_CODE.toString()
        || (promoPageState.codeType === PromoCodeType.CODE.toString() && !!promoPageState.codes)
        || (promoPageState.codeType === PromoCodeType.BAR_QR.toString() && !!promoPageState.imgCode ));
    ...
    steps[3].isCompleted = promoPageState.gender !== null
        ...
        && (promoPageState.interests && promoPageState.interests.length !== 0);
