...
interface IFacebook {
    Event: {
         ...
         * name: 'auth.statusChange' - fired when the status changes
    ...
    ui(params?: IFacebookUIParameters,
       cb?: (response?: any) => void): void;

    login(cb?: (response: IFacebookUserAuthenticate) => any,
          opts?: { scope: string; }): void;
    ...
    getLoginStatus(cb?: (response: IFacebookUserAuthenticate) => void,
                   // Access token of the user
                   force?: boolean): void;
